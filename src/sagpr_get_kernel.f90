    ...
    if (lm.eq.0) then
        ...
        do i=1,2
         ...
         if (i.eq.1) then
          ...
          nfeat_1 = reals / (nmol_1*natmax_1)
          call C_F_POINTER(C_LOC(raw_PS_1),PS_1,[nfeat_1,natmax_1,nmol_1])
         else
          allocate(raw_PS_2(reals))
          read(41,pos=1) raw_PS_2
          close(41)
          natmax_2 = maxval(natoms_2)
          nfeat_2 = reals / (nmol_2*natmax_2)
          call C_F_POINTER(C_LOC(raw_PS_2),PS_2,[nfeat_2,natmax_2,nmol_2])
         endif
        enddo
        ...
        if (zeta.eq.1) then
         ...
         do i=1,nmol_1
          do ii=1,int(natoms_1(i))
           PS_1_fast(:,i) = PS_1_fast(:,i) + PS_1(:,ii,i) / natoms_1(i)
          enddo
         enddo
         do i=1,nmol_2
          do ii=1,int(natoms_2(i))
           PS_2_fast(:,i) = PS_2_fast(:,i) + PS_2(:,ii,i) / natoms_2(i)
          enddo
         enddo
         ...
         do i=1,nmol_1
          ...
          do j=j0,nmol_2
           do ii=1,int(natoms_1(i))
            do jj=1,int(natoms_1(j))
             ker(i,j) = ker(i,j) + (dot_product(PS_1(:,ii,i),PS_2(:,jj,j)))**zeta
            enddo
           enddo
